...
#
# Also the vacuous-pass guard (formerly a separate inline workflow step): merge-base == HEAD means there
...
# NO BOUND, NO RESOLUTION. The fallback here used to `shift` the timeout off and run
# the command unbounded — which is the trade backwards: a git fetch against a hung
# remote then holds the whole lens job to its envelope instead of failing fast. This
# script already fails closed on no merge base, so an unbounded call is the one
# degradation that could hang rather than degrade.
if ! command -v timeout >/dev/null 2>&1; then
  echo "::error title=Cannot bound the merge-base resolution::coreutils timeout is unavailable, so a hung git operation could hold this job open. Failing closed. On a dev machine: install coreutils (macOS: brew install coreutils, and ensure gtimeout is on PATH as timeout)."
  exit 1
fi
bounded() { timeout "$@"; }
...
# `${VAR-10}` not `${VAR:-10}` — the colon form would swallow an explicitly-empty
# override into the default and leave the '' arm of the guard below unreachable.
GIT_OP_TIMEOUT="${GIT_OP_TIMEOUT-10}"
# Digits, and NOT ZERO — `timeout 0 cmd` means no limit. Validated because it is
# interpolated straight onto a command line.
case "$GIT_OP_TIMEOUT" in
  '' | *[!0-9]* | 0*)
    # Not echoed, for the reason the BASE_REF guard above does not echo its input:
    # an env-supplied value can carry workflow-command syntax, and this is a `::`
    # sink. Naming the variable says everything the operator needs.
    echo "::error title=git operation bound is invalid::GIT_OP_TIMEOUT must be a positive whole number of seconds. Failing closed."
    exit 1
    ;;
esac
