...
`KNOWN_UNCHECKED` array is the authoritative current list and count going forward; read it, not
this sentence's number, once more move. Picking one up means adding the missing narrowing checks
or a declaration file for the `.mjs` import, then moving that file from `KNOWN_UNCHECKED` into
