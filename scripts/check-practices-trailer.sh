...
#
# Only commits that touch source are required to carry the trailer — a
# docs/config/changelog-only commit is not a code change and is exempt.
#
# ADAPTED FOR THIS MONOREPO. The original matches `^src/.*\.ts$`, which is correct for a
# single-package repo and matches NOTHING here, where source lives at
# `packages/<name>/src/**.ts`. A pattern that matches nothing does not fail loudly — it
# reports success on every commit, which is indistinguishable from a gate that ran and
# found nothing wrong. If this repo grows a source root outside `packages/`, widen the
# pattern below or it will go quiet again.
set -euo pipefail
...
while read -r sha; do
  ...
  if ! git diff-tree --no-commit-id --name-only -r "$sha" | grep -qE '^packages/[^/]+/src/.*\.ts$'; then continue; fi
