 ...
 *
 * Branch-agnostic by construction: on `2S.4-promotion-workflow` the reachable set has no
 * `adapter-contract` and the assertion passes as things stand. On `story-2.2` the reachable
 * set includes it, so this test only passes once the Dockerfile carries it — which is the point.
 */
